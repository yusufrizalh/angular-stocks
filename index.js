...
app.get("/stocks/snapshot", function (req, res) {
  if (req.query.symbols) {
    ...
    res.status(400).send({
      message: `The request requires at least one symbol. Try adding '?symbols=appl' to the request.`,
    });
  }
});
...
app.get("/", function (req, res) {
  res
    ...
    .send(
      `Welcome to the Angular in Action API. See <a href='https://github.com/angular-in-action/api#readme'>https://github.com/angular-in-action/api#readme</a> for details.`
    );
});
...
setInterval(() => {
  ...
  stocks = stocks.map((stock) => {
    ...
    stock.changeInPercent =
      parseInt(stock.changeInPercent * 100 + change) / 100;
    stock.changeInPercent =
      parseInt(
        ((stock.change * 100) / (stock.lastTradePriceOnly * 100)) * 10000
      ) / 10000;
    return stock;
  });
